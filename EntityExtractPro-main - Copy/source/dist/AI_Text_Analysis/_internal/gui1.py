...
class NERApp:
    ...
    def setup_logging(self):
        logging.basicConfig(filename='ner_app.log', level=logging.INFO,
    ...
    def setup_gui(self):
        ...
        self.results_text = scrolledtext.ScrolledText(right_frame, wrap=tk.WORD, width=40, height=30)
    ...
    def _run_analysis_thread(self):
        try:
            ...
            self.root.after(0, self._update_results, entities)
            self.root.after(0, self.highlight_entities, doc)
    ...
    def _update_results(self, entities):
        self.results_text.delete(1.0, tk.END)
        categories = {}
        for entity, label in entities:
            if label not in categories:
                categories[label] = set()
            categories[label].add(entity)
    ...
    def highlight_entities(self, doc):
        if hasattr(self, 'text_widget'):
            ...
            for ent in doc.ents:
                start = self.text_widget.search(re.escape(ent.text), "1.0", tk.END)
                if start:
                    end = f"{start}+{len(ent.text)}c"
                    self.text_widget.tag_add(ent.label_, start, end)
                    self.text_widget.tag_config(ent.label_, background=self.get_color_for_label(ent.label_))
            self.text_widget.config(state=tk.DISABLED)
        elif self.file_path.endswith('.pdf'):
            self.highlight_pdf_entities(doc)

    def highlight_pdf_entities(self, doc):
        for page_num in range(self.total_pages):
            ...
            for ent in doc.ents:
                ...
                for rect in areas:
                    ...
                    highlight.set_colors(stroke=self.get_color_for_label(ent.label_, format="rgb"))
                    highlight.update()
        
        self.doc.save(self.file_path + "_highlighted.pdf")
        messagebox.showinfo("Highlighting Complete", "Highlighted PDF saved as: " + self.file_path + "_highlighted.pdf")
